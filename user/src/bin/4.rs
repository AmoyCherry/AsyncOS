...
pub fn main() -> i32 {
    ...
    println!("[user4 satp: {:#x}] main: Hello world from user mode program! time: {}", satp_read(), get_time());

    init_coroutine_interface();

    test_for_user();

    println!("[user4 satp: {:#x}] main: end, time: {}", satp_read(), get_time());
...
pub fn test_for_user(){
    ...
    unsafe{
        ...
        async fn test_num(prio: usize, no: usize) {
            println!("[hart {}] [user4 satp: {:#x}] prio: {}, No.: {}, total: 6", hart_id(), satp_read(), prio, no);
        }
