...
pub fn main() -> i32 {
    ...
    println!("[user4] main: Hello world from user mode program!");

    test_for_user();

    println!("[user4] main: end");
    ...
    unsafe{
        ...
        async fn test(x: i32) {
            let test_inner = async { 
                ...
                println!("[hart {}] [user4] calc plus, sum = {}", hart_id(), c); 
            };
            test_inner.await;
            println!("[hart {}] [user4] await done", hart_id()); 
        }
        ...
        async fn test_num(prio: usize, no: usize) {
            println!("[hart {}] [user4] prio: {}, No.: {}, total: 6", hart_id(), prio, no);
        }
